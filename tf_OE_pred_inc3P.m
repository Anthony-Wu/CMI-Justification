function [Y_p, Y_i] = tf_OE_pred_inc3P (U_i, F, B, dt, Y)
% OE predictor based on PharmaMV (infinite step, but starts when model is fully populated)
% MIMO compatible
...
for iY = 1:nY
	...
	Y_p(1:Cmax(iY),iY) = Y(1:Cmax(iY),iY);
	
	for isam = Cmax(iY)+1:nsam;
